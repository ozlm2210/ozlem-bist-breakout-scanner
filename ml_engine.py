...
from config import DATA_DIR, DEFAULT_WATCHLIST, CACHE_DAILY, YFINANCE_SUFFIX
...
def get_market_regime_classification(lookback_days: int = 365) -> dict:
    """
    Downloads BIST 100 (XU100.IS) daily data, runs KMeans clustering to identify
    the current market regime, and returns details.
    """
    try:
        # Download BIST 100 index data
        market = yf.download("XU100.IS", period="2y", progress=False, timeout=2.0)
        if market.empty:
            return {"name": "Unknown", "desc": "No market data available.", "color": "#9aa0a6"}
        
        market = _flatten_yfinance_cols(market)
        ...
        market_ema = compute_ema(close, 50)
        momentum = (close - market_ema) / market_ema
        ...
        try:
            ...
            if path.is_file():
                try:
                    ...
                    pass
            if df.empty:
                ticker_bist = f"{symbol}{YFINANCE_SUFFIX}"
                df = yf.download(ticker_bist, period="2y", progress=False, timeout=2.0)
                if df.empty:
                    failed_downloads += 1
                    continue
